...
uint256 hashGenesisBlock("0x162e4dd4dfa46ace5385b776ea4346de6aa0be000463e5ce87957ec5dc925d4b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Santacoin: starting difficulty is 1 / 2^12
...
{
    // Santacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // nSubsidy >>= (nHeight / 840000); // Santacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 60; // Santacoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Santacoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Santacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Santacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x88144e91db352ec9eb6810a80d3e7ea33c2ebec031247be8b2e4cd1c03d87bef");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Santa does not really exist 2019/03/11";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d1bdb5c9d0795542b9c8609c0e10abbbdf62dc046ebd085e2a20d91abe75079b37a71a3d33aca8f5346c47e7442e5abfbcc264814bf77c3c541f4e0769f8c575") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1552281422;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084605339;

        if (fTestNet)
        {
            block.nTime    = 1552281409;
            block.nNonce   = 387982877;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x54cd91876414e1bbde9936bc5c48eccbcabe4ede94207db0f867da947734841d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe1, 0xf2, 0xa3, 0xb4 }; // Santacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SantacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SantacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SantacoinMiner(CWallet *pwallet)
{
    printf("SantacoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running SantacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SantacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SantacoinMiner, pwallet));
}
